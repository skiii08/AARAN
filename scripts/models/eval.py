...
import torch
from .utils import denorm_by_user, mae_rmse, spearmanr_torch


@torch.no_grad()
def evaluate_raw(model, blobs, mu_map, std_map, device):
    """
    - model: モデル（v1/v2/v3 全対応）
    - blobs: hetero_graph_* dict
    - mu_map/std_map: user正規化パラメータ
    - 評価結果を (MAE, RMSE, ρ) で返す（rawスケール）
    """
    model.eval()

    user_feat = blobs["user_features"].to(device)
    movie_feat = blobs["movie_features"].to(device)

    u_idx = blobs["user_indices"].to(device)
    m_idx = blobs["movie_indices"].to(device)
    r_x   = blobs["review_signals"].to(device)
    y_raw = blobs["ratings_raw"].to(device)
    ...
    try:
        ...
        y_hat_norm = model(u_x, m_x, r_x, aspect_signals)
    except TypeError:
        # v1/v2 互換
        y_hat_norm = model(u_x, m_x, r_x)

    # denorm
    mu = torch.tensor([mu_map.get(int(u), mu_map[-1]) for u in u_idx.cpu().tolist()],
                      dtype=torch.float32, device=device)
    sd = torch.tensor([std_map.get(int(u), std_map[-1]) for u in u_idx.cpu().tolist()],
                      dtype=torch.float32, device=device)
    y_hat_raw = (y_hat_norm * sd + mu).clamp(1.0, 10.0)
    ...
    rx = torch.argsort(torch.argsort(y_hat_raw))
    ry = torch.argsort(torch.argsort(y_raw))
    rx, ry = rx.float(), ry.float()
    rho = float(((rx - rx.mean()) * (ry - ry.mean())).sum() /
                ((rx - rx.mean()).pow(2).sum().sqrt() * (ry - ry.mean()).pow(2).sum().sqrt() + 1e-8))
...
# =============================
# File: scripts/train/train_aaran_v1.py
# =============================
import torch
...
def main():
    ...
    mu_map, std_map = build_user_stats(train_blobs["user_indices"], train_blobs["ratings_raw"])  # for denorm

    model = AARANModel(AARANConfig())
    model.to(device)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if improved:
            ...
            torch.save({"model": model.state_dict(), "cfg": model.cfg.__dict__}, Path("outputs/aaran_v1_min.ckpt"))
            bad = 0
        else:
            bad += 1
        if bad >= patience:
